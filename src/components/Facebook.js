...
class FaceBook extends React.Component {
  ...
  showCountry = (country) => {
    let newProfilesArr =
      country === 'all'
        ? profiles
        : profiles.filter((state) => state.country === country);
    this.setState({
      profilesArr: newProfilesArr,
    });
  };
      ...
      <div>
        <div>
          ...
          {countries.map((country) => (
            <button onClick={() => this.showCountry(country)}>{country}</button>
          ))}
        </div>
          ...
          <div className="flex borderWrapper">
            ...
            <div>
              ...
              <p>
                ...
                {profiles.isStudent ? 'Student' : 'Teacher'}
              </p>
            </div>
          </div>
        ))}
